...
namespace Zadanie2Testy
{
    ...
    public class SerializationTest
    {
        ...
        [TestMethod]
        public void OwnFormatterTest_Recursion()
        {
            Container containerBeforeSerialization = new Container();
            ...
            containerBeforeSerialization.ListA.Add(a1);
            containerBeforeSerialization.ListB.Add(b1);
            containerBeforeSerialization.ListC.Add(c1);


            string path = "recursion.dat";

            OwnFormatter ownFormatter = new OwnFormatter();
            Stream serializationStream = File.Open(path, FileMode.Create, FileAccess.Write);
            ownFormatter.Serialize(serializationStream, containerBeforeSerialization);
            serializationStream.Close();


            Stream deserializationStream = File.Open(path, FileMode.Open, FileAccess.Read);
            Container containerAfterSerialization = ownFormatter.Deserialize(deserializationStream);
            deserializationStream.Close();


            Assert.AreEqual(containerAfterSerialization.ListA[0].GetC, containerAfterSerialization.ListC[0]);
            Assert.AreEqual(containerAfterSerialization.ListB[0].GetA, containerAfterSerialization.ListA[0]);
            Assert.AreEqual(containerAfterSerialization.ListC[0].GetB, containerAfterSerialization.ListB[0]);
        }
